...
function buildRedirectUrl(
    ...
    const fallbackUrl = new URL(`${redirectTo}?${searchParams}`, appUrl).toString();
        ...
        <script>
            if (window.opener) {
                window.opener.postMessage(
                    { type: 'OAUTH_CALLBACK', payload: ${JSON.stringify(params)} },
                    '${appUrl}'
                );
                // The main window will close this popup
            } else {
