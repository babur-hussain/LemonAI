...
function ChannelTabContent() {
    ...
    useEffect(() => {
        const handleMessage = (event: MessageEvent) => {
            ...
            if (event.data?.type === 'OAUTH_CALLBACK') {
                const { connected, error, channelType } = event.data.payload;
                ...
                if (connected === "true") {
                    toast.success(`Successfully connected to ${channelType || 'channel'}`);
                } else if (error) {
    ...
    const connectMutation = useMutation({
        ...
        onSuccess: ({ url }) => {
            ...
            window.open(url, "OAuthConnection", `width=${width},height=${height},top=${top},left=${left}`);
        },
        onError: (error: Error) => {
